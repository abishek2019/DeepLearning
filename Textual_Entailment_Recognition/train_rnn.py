...
class Model_2(NN.Module):
    ...
    # ----------------- YOUR CODE HERE ----------------------
        ...
        # input sentences representation 
        sents1 = self.embedding(rep1)
        sents2 = self.embedding(rep2)

        # (sequence length * batch size * feature size)
        sents1 = sents1.transpose(1, 0)
        sents2 = sents2.transpose(1,0)

        hidden1 = self.init_hidden(batch_size)

        # LSTM1 cell states container: sequence_length x batch_size x lstm size
        cells = torch.zeros(len(sents1), batch_size, self.lstm_size).type(self.float_type)

        for i in range(len(sents1)):
            hidden1 = self.lstm1(sents1[i], (torch.squeeze(hidden1[0]),torch.squeeze(hidden1[1])))
            cells[i] = hidden1[1]

        # (batch size * sequence length * feature size)
        cells = cells.transpose(1,0)

        # Get the valid last cell state 
        length1 = (len1-1).view(-1, 1, 1).expand(cells.size(0), 1, cells.size(2))

        cell = torch.gather(cells, 1, length1)
        cell = cell.view(1,cell.size(0), -1)
        hidden2 = self.init_hidden(batch_size)

        # Ouput of LSTM: sequence (length x mini batch x lstm size)
        lstm2_outs, hidden2 = self.lstm2(sents2, (hidden2[0],cell))

        # (Batch x Sequence_length x lstm_size)
        lstm2_outs = lstm2_outs.transpose(1, 0)

        # Last Output of LSTM 2
        length2 = (len2-1).view(-1, 1, 1).expand(lstm2_outs.size(0), 1, lstm2_outs.size(2))
        lstm2_out = torch.gather(lstm2_outs, 1, length2)

        # Convert batch input to 2D Tensor
        lstm2_out = lstm2_out.view(lstm2_out.size(0), -1)

        # Fully connected layer outputs 
        fc_out = F.dropout(F.tanh(self.linear1(lstm2_out)), p=self.drop_out)
        fc_out = F.dropout(F.tanh(self.linear2(fc_out)), p=self.drop_out)
        fc_out = F.dropout(F.tanh(self.linear3(fc_out)), p=self.drop_out)
        fc_out = self.linear4(fc_out)
...
class Model_3(NN.Module):
    ...
    def forward(self, rep1, len1, mask1, rep2, len2):
        ...
        # outputs_1: this is the matrix Y in the paper consisting of the LSMT1 output vectors (may be transposed).
        # WyY: this is the product between Y and the matrix Wy in the paper. Use it to compute M as in the paper.
        ...
        # LSTM 1: Initializing the hidden and cell states 
        hx, cx = torch.zeros(batch_size, self.lstm_size).type(self.float_type),\
                        torch.zeros(batch_size, self.lstm_size).type(self.float_type)
        
        # LSTM1 output container: sequence_length x batch_size x lstm size
        lstm1_hx = torch.zeros(max_seq_len, batch_size, self.lstm_size).type(self.float_type)
        
        for idx, premise in enumerate(sent1):
            # Initialization of hidden and cell state for LSTM 1
            hx, cx = torch.zeros(batch_size, self.lstm_size).type(self.float_type),\
                            torch.zeros(batch_size, self.lstm_size).type(self.float_type)
            hx, cx = self.lstm1(premise, (hx, cx))
            lstm1_hx[idx] = hx
       
       # (Batch * sequence length * feature size)
        output_1 = lstm1_hx.transpose(0,1)
                
        # LSTM 2: Initializing the hidden state
        hidden2 = self.init_hidden(batch_size)
        
        # LSTM2 ouput container: sequence (length x mini batch x lstm size)
        lstm_outs, hidden = self.lstm2(sent2, hidden2)
        ...
        h_t = torch.gather(lstm_outs, 1, len2)
        h_t = h_t.view(batch_size, self.lstm_size)
       
        
        WyY = torch.matmul(output_1,  self.Wy)
        r = context_vector(h_t, max_seq_len, output_1, WyY)
        
        # Sentence-pair final representation  
        h_repr = torch.tanh(torch.matmul(r, self.Wp) + torch.matmul(h_t, self.Wx))
        h_repr = h_repr.view(h_repr.size()[0],self.lstm_size)
       
        h_repr = F.dropout(h_repr, p=self.drop_out)
        
        # Fully connected layer
        fc_out = F.dropout(F.tanh(self.linear1(h_repr)), p=self.drop_out)
    ...
    def train_and_evaluate(self, FLAGS, embedding, train_set, dev_set, test_set):
        ...
        for i in range(num_epochs):
            ...
            while s < train_size:
                ...
                rep1 = torch.from_numpy(batch_1v).type(self.long_type)
                len1 = torch.from_numpy(batch_1l).type(self.long_type)
                rep2 = torch.from_numpy(batch_2v).type(self.long_type)
                len2 = torch.from_numpy(batch_2l).type(self.long_type)
                mask = torch.from_numpy(mask).type(self.long_type)
                label = torch.from_numpy(batch_label).type(self.long_type)
